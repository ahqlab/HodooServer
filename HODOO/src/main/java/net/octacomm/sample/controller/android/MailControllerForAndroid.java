...
@RequestMapping("/android/mail")
@Controller
public class MailControllerForAndroid {
	...
	public void mailSending() {
		...
		try {
			...
			messageHelper.setFrom(HodooConstant.EMAIL_FROM_ADDRESS, "호두스케일"); // 보내는사람 생략하거나 하면 정상작동을 안함
	...
	public int userCertifiedMailSend(
		...
		try {
			 ...
			 *  */
			String url = request.getRequestURL().toString().replace(request.getRequestURI(),"") + "/user/checkUserCertifiedMail.do?code=" + encodingStr;
			/* 보내는 url 생성 (e) */
			String htmlStr = "<html>" + 
